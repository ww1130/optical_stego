...
class model:
    def __init__(self,opt):
        ...
        self.net=Encoder.to(self.device)
...
class Encoder(nn.Module):
	...
	def forward(self, host, secret):
		...
		stego_res = self.iwt(stego_dwt_res)
...
def motion_blur(frame, kernel_size=5):
	...
	for i in range(frame.shape[0]):
		img = frame[i].permute(1, 2, 0).numpy()  # 调整维度顺序
		img = (img * 255).astype(np.uint8)  # 转换为uint8格式
		img_blurred = cv2.filter2D(img, -1, kernel)
		img_blurred = img_blurred.astype(np.float32) / 255.0
		blurred.append(torch.from_numpy(img_blurred).permute(2, 0, 1))  # 调整维度顺序
...
if __name__ == "__main__":
	...
	stego = encoder(host, secret)

	noiser = Noiser(noise_stddev=0.1, dct_quantization=0.5, conv_kernel_size=3, jpeg_quality=90, motion_kernel_size=5)
	distorted_residual_dwt_host = noiser(stego)


	print("Output shape:", stego.shape)
